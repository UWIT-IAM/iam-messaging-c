...
/* Send UW message to aws sqs queue */
...
void usage() {
   fprintf(stderr, "usage: %s [-c cfg_file] [-m message] [-f message_file] [-n num_msg] [-z] [-q queue]\n", prog);
   fprintf(stderr, "         [ -s  signid] [-d cryptid] \n");
   fprintf(stderr, "         -z  ( no crypt )\n");
   fprintf(stderr, "         -n  ( def = 1 )\n");
   exit (1);
}
...
int main(int argc, char **argv) {
   ...
   if (_queue) queue = iam_urlencode(_queue);
   if (queue) printf ("sending to: %s\n", queue);
   ...
   for (i=0;i<nsend;i++) {
      ...
      iam_msgSendSqs(ctx, msg, cryptid, signid);
   }
